---
name: send-capture
description: Capture a screenshot or video recording with agent-browser and send it inline via the send_capture tool. Use when the user asks to see something running, wants a screenshot or video of the demo, or needs visual proof of a component change.
---

# Send a Capture

Take a screenshot or screen recording with `agent-browser`, then send it with
the `send_capture` tool so it renders inline. Before driving the browser, run
`agent-browser skills get core` if you have not loaded the core guide in this
session.
...
Call the `send_capture` tool with the absolute capture path — never hand over
a bare path and assume it renders. Photos come back inline; videos return a
file path reference (inline video rendering lands in a later phase):

```
send_capture { path: "$CAP/<name>.png", label: "<subject>", kind: "photo" }
send_capture { path: "$CAP/<name>.webm", label: "<subject>", kind: "video" }
```

Photos are capped at 5MB — downscale or crop when the tool rejects a file.
...
yes, serve the captures dir with the bundled script and share it via bb
connect:

```bash
# from the thread that started the server (leave running while the link is needed):
node <this-skill>/scripts/serve-captures.mjs --port 8321 &
```

- Check `bb connect status --json` (must be paired + connected), then
  `bb connect expose 8321` from the thread that started the server. Give the
  returned URL as markdown links — never a localhost URL (it does not work
  remotely). `bb connect unexpose 8321` when the link is no longer needed.
...
# only if you exposed a share in step 3:
