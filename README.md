...
| [bb-plugin-opencode-go-usage](bb-plugin-opencode-go-usage/) | **OpenCode Go usage** — quota viewer for [opencode.ai/zen/go](https://opencode.ai). Shows 5-hour, weekly, and monthly usage via `bb opencode-go-usage`. Resolves `OPENCODE_API_KEY` from flag, plugin settings, `~/.pi/agent/auth.json`, or env; fetches `https://opencode.ai/zen/go/v1/usage`. |
