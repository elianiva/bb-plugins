...
Spawn all N workers in one parallel `subagent` call using its `tasks` array. Use `agent: "poteto-agent"` unless a workflow names another agent, and set `role: "swarm workers"`. BB child threads run locally with isolated context, so give each task explicit file pointers and prevent concurrent writes to shared paths.
