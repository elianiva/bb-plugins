...
Before spawning investigators, list the available MCPs from the agent environment. Use the available-tools map when present. Otherwise inspect the `mcps/` directory exposed for enabled MCP servers.
...
Launch all matching investigators in one parallel `subagent` call using its `tasks` array so they run concurrently. Don't ask one agent to cover multiple MCPs.

Subagent config (each):
- `agent`: `poteto-agent`
- `model`: your configured why-investigators model (default `inherit-parent`)
- use a normal child thread with full tool access. **Do not restrict tools.** It strips MCP access, which disables MCP-backed investigators entirely. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
Spawn one synthesizer subagent via the `subagent` tool:

- `agent`: `poteto-agent`
- `model`: your configured why-synthesizer model (default `inherit-parent`)
- use a normal child thread with full tool access. The synthesizer's quality check spot-verifies citations, which can require MCP access. Restricting tools defeats that.
