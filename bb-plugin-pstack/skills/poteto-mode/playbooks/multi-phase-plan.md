...
3. Explore in subagents with `agent: "poteto-agent"` via the `subagent` tool and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
5. Write under `/skill:technical-writing` in full, then apply `/skill:unslop`. The body is one Diátaxis mode, how-to. Appendices hold explanation and reference. Each heading states the task or the finding. No long dashes. No mid-sentence colons.
...
- [ ] Run the **unslop** skill over the diff before each commit and `/skill:no-comments` before review.
